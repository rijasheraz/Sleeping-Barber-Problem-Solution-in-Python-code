fromtkinter import *
fromtkinter.ttk import Progressbar, Style
importtkinter as tk
fromtkinter import messagebox
from threading import Semaphore, Condition
fromtkinter import scrolledtext
fromtkinter import Tk, Label, Canvas, Button, PhotoImage
import threading
import time
import queue
import random
...
defstart_loading(self, load, load_label, frame):
def update():
nonlocal i
if i <= 10:
...
defstart_barber_simulation(self):
...
ifself.customer_queue.empty():  
self.check_simulation_complete()

defcheck_simulation_complete(self):
self.text_widget.insert(tk.END, "Barber is going to sleep\n")
self.text_widget.yview(tk.END)
time.sleep(random.uniform(1, 1))  # Simulate barber going to sleep
...
defshow_next_page_func_no(self):
